...
export const adminSendSMSToUser = api<SendSMSToUserRequest, SendSMSResponse>(
  ...
  async (req) => {
    ...
    const user = await db.queryRow<{ phone: string | null }>`
    ...
    await sendSMS(normalised, req.message.trim());
...
export const adminSendBulkSMS = api<SendBulkSMSRequest, SendSMSResponse>(
  ...
  async (req) => {
    ...
    const users = await db.queryAll<{ user_id: string; phone: string | null }>`
      SELECT u.user_id,
    ...
    for (const user of users) {
      ...
      try {
        await sendSMS(phone, req.message.trim());
        sent++;
      } catch (e: unknown) {
        status = "failed";
