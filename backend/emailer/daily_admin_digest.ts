  ...
  async (): Promise<void> => {
    ...
    const [
      ...
      db.queryRow<{ count: number }>`
        SELECT COUNT(*)::int AS count FROM employer_subscriptions es JOIN employers e ON e.employer_id = es.employer_id JOIN users u ON u.user_id = e.user_id WHERE es.status = 'active' AND u.is_demo = FALSE
      `,
      db.queryRow<{ total: number }>`
        SELECT COALESCE(SUM(es.amount_aud_cents),0)::int AS total FROM employer_subscriptions es JOIN employers e ON e.employer_id = es.employer_id JOIN users u ON u.user_id = e.user_id WHERE es.status = 'active' AND u.is_demo = FALSE
      `,
      db.queryRow<{ count: number }>`
      ...
      db.queryRow<{ active: number; total: number; new_today: number }>`
        SELECT
          COUNT(*) FILTER (WHERE j.status='Open')::int AS active,
          COUNT(*)::int AS total,
          COUNT(*) FILTER (WHERE j.created_at >= NOW() - INTERVAL '1 day')::int AS new_today
        FROM job_requests j JOIN employers e ON e.employer_id = j.employer_id JOIN users u ON u.user_id = e.user_id WHERE u.is_demo = FALSE
      `,
      ...
      `,
      db.queryRow<{ count: number }>`
        SELECT COUNT(*)::int AS count FROM offers o JOIN job_requests j ON j.job_id = o.job_id JOIN employers e ON e.employer_id = j.employer_id JOIN users u ON u.user_id = e.user_id WHERE o.created_at >= NOW() - INTERVAL '1 day' AND u.is_demo = FALSE
      `,
      db.queryRow<{ count: number }>`
        SELECT COUNT(*)::int AS count FROM messages m JOIN users u ON u.user_id = m.sender_user_id WHERE m.created_at >= NOW() - INTERVAL '1 day' AND u.is_demo = FALSE
      `,
    ]);
