...
export interface ListJobRequestsResponse {
  jobs: JobRequest[];
}

export const listJobRequests = api<void, ListJobRequestsResponse>(
  { expose: true, auth: true, method: "GET", path: "/jobs" },
  async () => {
    const auth = getAuthData()!;
    if (auth.role !== "EMPLOYER") {
      throw APIError.permissionDenied("only employers can list job requests");
    }
