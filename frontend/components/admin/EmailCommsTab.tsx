...
import {
  ...
  ImagePlus, Copy, Check, ChevronDown, ChevronUp,
} from "lucide-react";
...
type CommsTab = "templates" | "send" | "log";
...
export function EmailCommsTab({ api }: { api: ReturnType<typeof useAuthedBackend> }) {
  ...
  const tabs: { id: CommsTab; label: string; icon: React.ReactNode }[] = [
    ...
    { id: "log",       label: "Sent Log",   icon: <Mail className="h-3.5 w-3.5" /> },
  ];
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center gap-2">
        ...
        <div>
          <h2 className="text-sm font-semibold text-foreground">Email Communications</h2>
          <p className="text-xs text-muted-foreground">Manage templates and send emails to users</p>
        </div>
      </div>
